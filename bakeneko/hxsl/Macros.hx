...
class Macros {
	...
	static function makeType( t : Type ) : ComplexType {
		...
		case TVec(_, t):
			switch( t ) {
			case VFloat:
				macro : bakeneko.hxsl.Types.Vec;
			case VInt:
				macro : bakeneko.hxsl.Types.IVec;
			case VBool:
				macro : bakeneko.hxsl.Types.BVec;
			}
		case TStruct(vl):
			TAnonymous([for( v in vl ) { pos : Context.currentPos(), name : v.name, kind : FVar(makeType(v.type)) } ]);
		case TSampler2D:
			macro : bakeneko.hxsl.Types.Sampler2D;
		case TSamplerCube:
			macro : bakeneko.hxsl.Types.SamplerCube;
		case TMat3, TMat3x4, TMat4:
			macro : bakeneko.hxsl.Types.Matrix;
		case TString:
			macro : String;
		case TInt, TBytes(_):
			macro : Int;
	...
	static function makeDef( t : Type, pos : Position ) : haxe.macro.Expr {
		...
		case TVec(_, t):
			switch( t ) {
			case VFloat:
				macro new bakeneko.hxsl.Types.Vec();
			case VInt:
				macro new bakeneko.hxsl.Types.IVec();
			case VBool:
				macro new bakeneko.hxsl.Types.BVec();
			}
		case TStruct(vl):
		...
		case TArray(_):
			macro new Array();
		case TMat3, TMat3x4, TMat4:
			macro new bakeneko.hxsl.Types.Matrix();
		default:
			null;
		}
	}
	...
	static function buildFields( shader : ShaderData, pos : Position ) {
		...
		fields.push( {
			...
			kind : FFun({
				ret : macro : Void,
				args : [ { name : "globals", type : macro : bakeneko.hxsl.Globals } ],
				expr : macro {
					constBits = 0;
					{$a{ exprs }};
		...
		if( params.length > 0 ) {
			...
			fields.push({
				...
				pos : pos,
				kind : FFun({
					ret : macro : bakeneko.hxsl.Shader,
					args : [],
		...
		fields.push( {
			name : "_SHADER",
			kind : FVar(macro : bakeneko.hxsl.SharedShader),
			pos : pos,
			access : [AStatic],
			...
			for( m in f.meta ) {
				if( m.name == "global" )
					...
					case [FVar(t, set), EConst(CString(name))]:
						...
						fields.push({
							name : id,
							kind : FVar(macro : bakeneko.hxsl.Globals.GlobalSlot<$t>),
							pos : f.pos,
						...
						globals.push(macro $i{id} = new bakeneko.hxsl.Globals.GlobalSlot($v{ name }));
						if( set != null )
